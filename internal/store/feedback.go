...
func (s *Store) UpsertUser(ctx context.Context, provider, externalID, username, name, avatarURL string, trustLevel int) (User, error) {
	provider = strings.TrimSpace(provider)
	externalID = strings.TrimSpace(externalID)
	username = strings.TrimSpace(username)
	if provider == "" || externalID == "" || username == "" || len(provider) > 100 || len(externalID) > 200 || len(username) > 200 || len(name) > 200 || len(avatarURL) > 1000 || trustLevel < 0 || trustLevel > 10 {
