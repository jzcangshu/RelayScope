...
1. **每个阶段独立提交、测试全绿后才进下一步。** 每步必须通过 `go vet ./...`、`go test ./...`、`gofmt -l .`；Phase 3 起同时运行 Node 前端行为测试。
...
Phase 0-4 完成后的独立审计已收口：采集完成和适配器调用具备取消/崩溃恢复边界；站点创建和管理更新拒绝未注册适配器；HTTP 错误保留分类能力但不把 query 凭据写入运行记录；启动时会回收进程中断的 `running` 采集记录；readiness 会检查 SQLite；公开页面主题初始化改为 CSP 兼容的嵌入外部脚本。调度器按最多 100 个 due 站点批次工作，符合当前数十站点容量目标；若部署规模超过该边界，应先做容量测量再调整。
